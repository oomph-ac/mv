...
func (blockPaletteEncoding) encode(buf *bytes.Buffer, v uint32) {
	...
	name, props, _ := latest.RuntimeIDToState(v)
	_ = nbt.NewEncoderWithEncoding(buf, nbt.LittleEndian).Encode(latest.State{Name: name, Properties: props, Version: CurrentBlockVersion})
}
func (blockPaletteEncoding) decode(buf *bytes.Buffer) (uint32, error) {
	...
	v, ok := latest.StateToRuntimeID(e.Name, e.Properties)
	if !ok {
...
func (networkPersistentEncoding) encodePalette(buf *bytes.Buffer, p *Palette, _ paletteEncoding) {
	...
	for _, val := range p.values {
		name, props, _ := latest.RuntimeIDToState(val)
		_ = enc.Encode(latest.State{Name: strings.TrimPrefix("minecraft:", name), Properties: props, Version: CurrentBlockVersion})
	}
}
func (networkPersistentEncoding) decodePalette(buf *bytes.Buffer, blockSize paletteSize, _ paletteEncoding) (*Palette, error) {
	...
	blocks := make([]latest.State, paletteCount)
	...
	for i, b := range blocks {
		temp, ok = latest.StateToRuntimeID("minecraft:"+b.Name, b.Properties)
		if !ok {
